...
def render_top_navigation(modul_listesi, active_index, label, engine):
    """v8.9.3: Stabil navigasyon - quick_nav sadece display, modül değiştirmez"""
    ...
    with c2:
        ...
        with c2_1:
            # v8.9.3: DISPLAY-ONLY selectbox - does NOT trigger module changes
            # Navigation is handled exclusively by sidebar_nav radio and buttons
            # Selectbox key is "quick_nav_display" (not "quick_nav") so app_nav_sync ignores it
            selected_lbl = st.selectbox(
                "🚀 HIZLI GEÇİŞ",
                modul_listesi,
                index=active_index,
                key="quick_nav_display",
                label_visibility="collapsed"
            )
            # Kullanıcı bu selectbox'tan seçim yaptıysa module değiştir
            # active_index ile karşılaştır (her ikisi de indeks-tabanlı)
            if selected_lbl != modul_listesi[active_index]:
                lbl_to_slug = st.session_state.get('_lbl_to_slug_map', {})
                new_slug = lbl_to_slug.get(selected_lbl)
                if new_slug:
