...
def get_aktif_vardiya(engine, makina_no=None) -> dict | None:
    ...
    if makina_no:
        sql = "SELECT id, tarih, makina_no, vardiya_no, operator_adi, durum, baslangic_saati FROM map_vardiya WHERE durum='ACIK' AND makina_no=:m ORDER BY id DESC LIMIT 1"
        params = {"m": makina_no}
    else:
        sql = "SELECT id, tarih, makina_no, vardiya_no, operator_adi, durum, baslangic_saati FROM map_vardiya WHERE durum='ACIK' ORDER BY id DESC LIMIT 1"
        params = {}
...
def get_gunluk_vardiyalar(engine, tarih: str) -> pd.DataFrame:
    ...
    sql = "SELECT id, makina_no, vardiya_no, durum, baslangic_saati FROM map_vardiya WHERE tarih=:t ORDER BY makina_no ASC, id DESC"
    with engine.connect() as conn:
...
def get_tum_aktif_vardiyalar(engine) -> pd.DataFrame:
    ...
    sql = "SELECT id, makina_no, vardiya_no, durum, baslangic_saati FROM map_vardiya WHERE durum='ACIK' ORDER BY makina_no ASC"
    with engine.connect() as conn:
