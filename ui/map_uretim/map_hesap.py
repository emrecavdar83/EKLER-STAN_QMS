...
def hesapla_sure_ozeti(engine, vardiya_id: int) -> dict:
    ...
    df = pd.read_sql(
        text("SELECT * FROM map_zaman_cizelgesi WHERE vardiya_id=:v"),
        engine, params={"v": vardiya_id}
    )
    vdf = pd.read_sql(
        text("SELECT * FROM map_vardiya WHERE id=:v"),
        engine, params={"v": vardiya_id}
    )
    if vdf.empty:
        return {}
    ...
    mola_dk = float(df[df['neden'].astype(str).str.contains('Mola', na=False)]['sure_dk'].fillna(0).sum())
...
def hesapla_uretim(engine, vardiya_id: int) -> dict:
    ...
    vdf = pd.read_sql(
        text("SELECT hedef_hiz_paket_dk, gerceklesen_uretim FROM map_vardiya WHERE id=:v"),
        engine, params={"v": vardiya_id}
    )
    if vdf.empty:
        return {}
    ...
    fire_df = pd.read_sql(
        text("SELECT SUM(miktar_adet) as toplam FROM map_fire_kaydi WHERE vardiya_id=:v"),
        engine, params={"v": vardiya_id}
    )
...
def hesapla_durus_ozeti(engine, vardiya_id: int) -> list:
    ...
    df = pd.read_sql(
        text("""SELECT neden, COUNT(*) as olay_sayisi, SUM(sure_dk) as toplam_dk
...
def hesapla_fire_ozeti(engine, vardiya_id: int) -> list:
    ...
    df = pd.read_sql(
        text("""SELECT fire_tipi, SUM(miktar_adet) as miktar
