...
def uret_is_raporu_html(engine, vardiya_id: int):
    ...
    Format: EKL-URT-R-MAP-001 (HTML/A4 Standardı)
     Çoklu Makine (13. Adam): Aynı shiftteki tüm makineleri tek PDF'te birleştirir.
    """
    from ..raporlama_ui import _generate_base_html
    
    # 1. Ana (Tetikleyici) Vardiya Bilgisi (Başlıklar için)
    with engine.connect() as conn:
        df_base = db._read(conn, "SELECT * FROM map_vardiya WHERE id=:id", {"id": vardiya_id})
        if df_base.empty: return None
        base_v = df_base.iloc[0].to_dict()
        
    period = f"{base_v['tarih']} | {base_v['vardiya_no']}. Vardiya"
    
    # İlgili tüm makineleri bul (Aynı tarih ve vardiya_no)
    ...
    for v_id in related_ids:
        # Veri Hazırlama
        with engine.connect() as conn:
            df_v = db._read(conn, "SELECT * FROM map_vardiya WHERE id=:id", {"id": v_id})
            if df_v.empty: continue
            v = df_v.iloc[0].to_dict()
            
        ozet = hesap.hesapla_sure_ozeti(engine, v_id)
        uretim = hesap.hesapla_uretim(engine, v_id)
        duruslar = hesap.hesapla_durus_ozeti(engine, v_id)
        fireler = hesap.hesapla_fire_ozeti(engine, v_id)
        df_b = db.get_bobinler(engine, v_id)
        df_z = db.get_zaman_cizelgesi(engine, v_id)

        # Makine Özel KPI Kartları
        machine_summary = f"""
        <div class="ozet-bar" style="margin-bottom:15px; margin-top:10px;">
        ...
        # Makine Özel HTML Bloğu
        makina_block = f"""
        <div style="page-break-inside: avoid; margin-bottom: 30px; border: 2px solid #1a2744; border-radius: 8px; padding: 15px;">
            <h2 style="color:white; background-color:#1a2744; padding:10px; margin:-15px -15px 15px -15px; border-radius: 6px 6px 0 0; text-align:center;">
                {v['makina_no']} MAKİNESİ VERİLERİ
            </h2>
            ...
            <h3 style="color:#1a2744; border-left:4px solid #8B0000; padding-left:10px; font-size:14px;">ÇALIŞMA & DURUŞ ÇİZELGESİ</h3>
            <table>
            ...
            <div style="display:flex; gap:15px;">
                <div style="flex:1;">
                    <h3 style="color:#1a2744; border-left:4px solid #8B0000; padding-left:10px; font-size:14px;">DURUŞ ANALİZİ</h3>
                    <table>
                ...
                <div style="flex:1;">
                    <h3 style="color:#1a2744; border-left:4px solid #8B0000; padding-left:10px; font-size:14px;">FİRE ANALİZİ</h3>
                    <table>
            ...
            <h3 style="color:#1a2744; border-left:4px solid #8B0000; padding-left:10px; font-size:14px;">BOBİN DEĞİŞİM KAYDI (KG)</h3>
            <table>
    ...
    # Toplu rapor olduğu için en üstte summary_cards boş gönderiliyor, içerik dinamik oldu
    html = _generate_base_html("MAP MAKİNELERİ ÜRETİM İŞ RAPORU", "EKL-URT-R-MAP-001", period, "", master_content, signatures)
    return html
