...
def qdms_dokuman_merkezi_content(engine=None):
    ...
    tip_filter = col2.selectbox("Belge Tipi", ["Tümü", "GK", "SO", "TL", "PR", "KYS", "FR", "PL", "GT", "LS", "KL", "YD", "SOP"], key="dm_tip")
    ...
    for _, row in df.iterrows():
        with st.container(border=True):
            ...
            color = {"aktif": "green", "taslak": "gray", "incelemede": "orange", "arsiv": "red"}.get(row['durum'], "gray")
...
def qdms_belge_yonetimi_content(engine=None):
    ...
    with st.expander("➕ YENİ BELGE OLUŞTUR", expanded=False):
        with st.form("yeni_belge_form"):
            c1, c2 = st.columns(2)
            b_tip = c1.selectbox("Belge Tipi", ["GK", "SO", "TL", "PR", "KYS", "FR", "PL", "LS", "KL", "SOP"])
            ...
            if st.form_submit_button("Oluştur"):
                ...
                if res['basarili']: st.success(f"Belge {b_kod} oluşturuldu."); st.rerun()
...
def qdms_talimat_content(engine=None):
    ...
    tab1, tab2 = st.tabs(["Talimat Oluştur", "Onay Bekleyenler"])
    with tab1:
        with st.form("talimat_form"):
            tk = st.text_input("Kod (EKL-TL-001)")
            ta = st.text_input("Adı")
            adimlar = st.text_area("Adımlar (JSON)", value="[]")
            if st.form_submit_button("Kaydet"):
                try:
                    res = talimat_olustur(engine, tk, ta, "Genel", json.loads(adimlar))
                    if res['basarili']: st.success("Talimat kaydedildi."); st.rerun()
...
def qdms_main_page(engine=None):
    ...
    t1, t2, t3, t4 = st.tabs(["📋 Doküman Merkezi", "⚙️ Yönetim", "📖 Talimatlar", "📊 Uyumluluk"])
...
def _render_belge_editor(engine, row):
    ...
    if is_gk:
        ...
        with st.form(f"doc_edit_{row['belge_kodu']}"):
            ...
            e_amac = st.text_area("Amaç", value=current.get('amac', ''))
            e_icerik = st.text_area("İçerik / Uygulama", value=current.get('icerik', ''))
            if st.form_submit_button("💾 GÜNCELLE"):
                res = belge_guncelle(engine, row['belge_kodu'], new_ad, current['alt_kategori'], "", amac=e_amac, icerik=e_icerik)
                if res['basarili']: st.success("Güncellendi."); st.rerun()
