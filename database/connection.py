...
def _bootstrap_map_parameters(conn, is_pg):
    """Anayasa v5.8.9: MAP Duruş ve Fire tiplerini koda gömülü halden DB'ye aktarır (Sıfır Hardcode)."""
    try:
        _pk = "SERIAL PRIMARY KEY" if is_pg else "INTEGER PRIMARY KEY AUTOINCREMENT"
        conn.execute(text(f"CREATE TABLE IF NOT EXISTS map_durus_nedenleri (id {_pk}, neden TEXT UNIQUE NOT NULL, aktif INTEGER DEFAULT 1)"))
        conn.execute(text(f"CREATE TABLE IF NOT EXISTS map_fire_tipleri (id {_pk}, fire_tipi TEXT UNIQUE NOT NULL, aktif INTEGER DEFAULT 1)"))
        
        duruslar = ["ÜST FİLM DEĞİŞİMİ", "ALT FİLM DEĞİŞİMİ", "MOLA / YEMEK", "ARIZA / BAKIM", "SETUP / AYAR", "ÜRETİM BEKLEME", "TEMİZLİK / SANİTASYON", "DİĞER"]
        fireler = ["Bobin Başı Fire", "Bobin Sonu Fire", "Film Değişimi Fire", "Sızdırmazlık / Kaçak", "Yırtık / Delik Film", "Gaz Hatası", "Besleme Hatası", "Operatör Hatası", "Diğer"]
        ...
        for f in fireler:
            sql = "INSERT INTO map_fire_tipleri (fire_tipi) VALUES (:f)"
            if is_pg: sql += " ON CONFLICT (fire_tipi) DO NOTHING"
            else: sql = sql.replace("INSERT INTO", "INSERT OR IGNORE INTO")
            conn.execute(text(sql), {"f": f})
    except Exception as e:
        print(f"Bootstrap MAP Parameters Warning: {e}")
