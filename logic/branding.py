...
def _premium_css() -> str:
    """Kurumsal UI için Inter font + glassmorphism header + sidebar CSS'ini döndürür."""
    return """
        ...
        <style>
            html, body, [class*="css"] { font-family: 'Inter', sans-serif !important; background-color: #f8f9fa; }
            h1, h2, h3, .stHeader     { font-family: 'Outfit', sans-serif !important; color: #1e293b; }
            #MainMenu {visibility: hidden;} footer {visibility: hidden;} header {visibility: hidden;} .stDeployButton {display:none;}
            .corporate-header {
                position: fixed; top: 0; left: 0; width: 100%; z-index: 999996;
                padding: 10px 30px; background: rgba(255,255,255,0.85);
                backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px);
                border-bottom: 1px solid rgba(180,11,11,0.2);
                display: flex; align-items: center; justify-content: space-between; box-shadow: none !important;
            }
            .header-left  { display: flex; align-items: center; gap: 15px; }
            .header-logo  { height: 42px; width: auto; }
            .header-title {
                font-family: 'Outfit', sans-serif; font-weight: 700; font-size: 1.4rem;
                color: #b40b0b; letter-spacing: 1.5px; margin: 0;
                padding-left: 15px; border-left: 2px solid #b40b0b;
            }
            div.stButton > button       { border-radius: 8px !important; font-weight: 600 !important; transition: all 0.3s ease !important; border: none !important; box-shadow: none !important; }
            div.stButton > button:hover { transform: translateY(-2px) !important; box-shadow: none !important; background-color: #a00a0a !important; }
            [data-testid="stSidebar"]   { background-color: #ffffff !important; border-right: 1px solid #e2e8f0 !important; }
            [data-testid="stSidebar"] [data-testid="stMarkdownContainer"] p,
            [data-testid="stSidebar"] span,
            [data-testid="stSidebar"] label { color: #1e293b !important; }
            [data-testid="stSidebar"] [data-testid="stIcon"] { color: #b40b0b !important; }
...
def set_branding():
    """Kurumsal font + glassmorphism CSS'ini Streamlit sayfasına enjekte eder."""
