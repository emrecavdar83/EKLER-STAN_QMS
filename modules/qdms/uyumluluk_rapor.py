    ...
    try:
        if hasattr(db_conn, 'execute'):
            ...
            rev_count = db_conn.execute(text("""
                SELECT COUNT(*) FROM qdms_revizyon_log 
                WHERE degisiklik_tarihi > (SELECT datetime('now', '-30 days'))
            """)).scalar()
            
            # Eskiyen belgeler (12 aydan eski revizyonu olan aktif belgeler)
            eskiyen = db_conn.execute(text("""
                SELECT belge_kodu, belge_adi, guncelleme_tarihi FROM qdms_belgeler
                WHERE durum = 'aktif' AND guncelleme_tarihi < (SELECT datetime('now', '-365 days'))
            """)).fetchall()
            
        else:
            with db_conn.connect() as conn:
                stats = conn.execute(text("SELECT COUNT(CASE WHEN durum = 'aktif' THEN 1 END) as aktif, COUNT(CASE WHEN durum = 'taslak' THEN 1 END) as taslak, COUNT(CASE WHEN durum = 'arsiv' THEN 1 END) as arsiv FROM qdms_belgeler")).fetchone()
                rev_count = conn.execute(text("SELECT COUNT(*) FROM qdms_revizyon_log WHERE degisiklik_tarihi > (SELECT datetime('now', '-30 days'))")).scalar()
                eskiyen = conn.execute(text("SELECT belge_kodu, belge_adi, guncelleme_tarihi FROM qdms_belgeler WHERE durum = 'aktif' AND guncelleme_tarihi < (SELECT datetime('now', '-365 days'))")).fetchall()
